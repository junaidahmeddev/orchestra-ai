...
export async function PUT(req: Request, { params }: RouteParams) {
  try {
    ...
    const updatedWorkflow = await db.workflow.update({
      ...
      data: {
        ...dataToUpdate,
        canvasJson: dataToUpdate.canvasJson ? (dataToUpdate.canvasJson as Prisma.InputJsonValue) : undefined,
      },
    });
...
export async function DELETE(req: Request, { params }: RouteParams) {
  try {
    ...
    // Cleanly delete child execution runs, edges, and nodes first to prevent FK constraint failures
    await db.$transaction([
      db.nodeRun.deleteMany({
        where: { workflowRun: { workflowId: id } },
      }),
      db.workflowRun.deleteMany({
        where: { workflowId: id },
      }),
      db.workflowEdge.deleteMany({
        where: { workflowId: id },
      }),
      db.workflowNode.deleteMany({
        where: { workflowId: id },
      }),
      db.workflow.delete({
        where: { id },
